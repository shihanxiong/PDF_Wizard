...
- Returns the stored code when it matches `validLanguages` loaded from embedded `i18n/supported-languages.json` (same file as frontend `SUPPORTED_LANGUAGES`)
...
- Validates the code against `validLanguages` from embedded `i18n/supported-languages.json`
...
func (a *App) ExtractPDFText(path string) (string, error) {
